...
REGEXPS = {
    ...
    'stats result': r'.*{"action":"(test\w*)".*(?:Sent|Received)',
    ...
    try:
        while 1:
            ...
            if REGEXPS['stats block'].match(line):
                ...
                timestamp, passed, session_failures = process_stats_block(
                    reader, client_results)
                client_results['session failures'] += session_failures
...
def process_stats_block(reader, client_results, just_scan=False):
    ...
    while 1:
        ...
        m = REGEXPS['stats result'].match(line)
        if m:
            if m.group(1) != 'test_pass':
                block['failed tests'].append((number, line))
        elif REGEXPS['test failure'].match(line):
            # Some other failure printed out asynchronously during stats block
            session_failures.append((number, line))
        else:
            requeue_line(reader)
            break
    ...
    return block['timestamp'], not block['failed tests'], session_failures
...
def parse(filename):
    ...
    try:
        ...
        # We didn't finish processing, return what we have
        pass
